...
5. The user selects a leaf variable such as `consumerFeed.hh` or `consumerFeed.m`.
6. The selected variable is added to the active plot view.

### Expected Behavior

- Parent nodes are expandable and do not need to be selected as data series.
- Leaf nodes represent selectable variables.
...
3. The app shows the available parameters and their source binding.
4. The user assigns one or more parameters to the current plot.
